...
def generate():
    ...
    try:
        logger.info(f"saving features to {UNCOMPRESSED_FEATURES} and file names to {FILENAMES}")
    ...
    try:
        logger.info(f"Loading {UNCOMPRESSED_FEATURES}")
        feature_list = np.array(pickle.load(open(UNCOMPRESSED_FEATURES, "rb")))
        logger.info(f"loading {UNCOMPRESSED_FEATURES} successful")
    except Exception as e:
    ...
    try:
        logger.info(f"Saving compressed features to {COMPRESSED_FEATURES}")
    ...
    try:
        logger.info(f"Generating annoy index to {ANNOY}")
